...
import { axisDefaults, color, font, seriesDefaults } from '../plotUtil';
...
export default function LoadFile({ path }: { path: string }) {
	...
	return (<>
		...
		{open && <>
			...
			<div className='popup' style={{ left: '50%', top: '40%', padding: '0 2em' }}>
					...
					<UplotReact {...{
						...
						options: {
							...
							axes:[ {
								...
								...axisDefaults(),
								values: (u, valus) => valus.map(v => v.toString())
							} ],
							series: [ {
				...
				{data && <button style={{ marginTop: 16, padding: '2px 16px' }}
					onClick={() => mutation.mutate()}>Upload {targetVar} to server (overwrite)</button> }
				<div style={{ color: color('red'), height: '1.5em' }}>{report}</div>
