...
from data_source.atmosphere.download import Downloader
...
downloaders = dict({
    'temperature': Downloader('pressure', lambda y: f'air.{y}.nc'),
    'gflux': Downloader('surface_gauss', lambda y: f'gflux.sfc.gauss.{y}.nc')
})
...
def _obtain_year(what, dt_from, dt_to, merge_query=None):
    try:
        ...
        res = _extract_from_file(what, fname, dt_from, dt_to)
        ...
        query = downloaders[what].download(year)
        if merge_query:
            merge_query(query)
        query.await_result()
        res = _extract_from_file(what, fname, dt_from, dt_to)
...
def obtain(what, t_from, t_to, mq):
    ...
    times, data = _obtain_year(what, dt_from, datetime(dt_from.year, 12, 31, 18), mq)
    time_acc = [times]; data_acc = [data]
    for year in range(dt_from.year + 1, dt_to.year): # extract fully covered years
        times, data = _obtain_year(what, datetime(year, 1, 1, 0), datetime(year, 12, 31, 18), mq)
        time_acc.append(times)
        data_acc.append(data)
    times, data = _obtain_year(what, datetime(dt_to.year, 1, 1, 0), dt_to, mq)
    time_acc.append(times)
    data_acc.append(data)
