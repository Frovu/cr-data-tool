...
def upsert_many(conn, table, columns, data, conflict_constraint='time', do_nothing=False):
	with conn.cursor() as cur, conn.transaction():
		...
		cur.execute(f'INSERT INTO {table}({",".join(columns)}) SELECT {",".join(columns)} FROM tmp ' +
			...
			f'ON CONFLICT ({conflict_constraint}) DO UPDATE SET ' +
				 ','.join([f'{c} = COALESCE(EXCLUDED.{c}, {table}.{c})' for c in columns if c not in conflict_constraint])))
