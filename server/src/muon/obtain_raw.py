...
def _obtain_moscow(t_from: int, t_to: int, station: str, what: str):
	...
	logging.debug(f'Muones: got raw [{len(result)}/{(t_to-t_from)//60+1}] {station}:{what} {t_from}:{t_to}')
	return result

def _obtain_apatity(t_from, t_to, station, what='source'):
	...
	logging.debug(f'Muones: got raw [{len(result)}/{(t_to-t_from)//60+1}] {station} {t_from}:{t_to}')
	return result
