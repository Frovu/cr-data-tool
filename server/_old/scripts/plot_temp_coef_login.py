...
def _obtain_coef(dt_from, dt_to, station, channel, single):
    ...
    while True:
        ...
        if status != 'ok':
            ...
            return dt_from, data.get('coef' if single else 'coef_temperature'), 0#data.get('error')
...
if __name__ == '__main__':
    tfr = datetime(1990, 1, 5)
    tto = datetime(2012, 1, 5)
    fig, ax = plt.subplots()
    channels = ['V', 'N2']
    colors = ['#00FFFF', '#00FF88', '#00AAFF', '#00AAAA']

    times, coef, err = obtain('Nagoya', 'V', tfr, tto, timedelta(days=600), single=True)
    coef /= -100
    ax.errorbar(times, coef, err, fmt=f'-', color='#FFAA00', ecolor='magenta', label=f't_coef :V (single)')
    for channel in channels:
        times, coef, err = obtain('Nagoya', channel, tfr, tto, timedelta(days=600))
        print(coef)
